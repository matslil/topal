...
`TOPAL-COMPILER-LEXICAL-RETURN-COMPLETE-DECISION-SUBJECT-001`, and complete
...
`TOPAL-COMPILER-LEXICAL-RETURN-ORDERED-COMPARISON-DECISION-ACTIONS-001`, and
final-fallback declared-Enum decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-ENUM-FALLBACK-DECISION-ACTIONS-001`, and
exhaustive declared-Enum decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-ENUM-EXHAUSTIVE-DECISION-ACTIONS-001`, and
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete
List decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, a
...
exit-edge and cleanup lowering is implemented.
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete
List decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, this rule
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete
List decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, other
...
`TOPAL-COMPILER-LEXICAL-RETURN-UNORDERED-COLLECT-001`, the infix Array and String
forms covered by `TOPAL-COMPILER-LEXICAL-RETURN-INFIX-COLLECT-001`, and the
...
`collect-map` covered by `TOPAL-COMPILER-LEXICAL-RETURN-MAP-COLLECT-001` and the
infix Array and String forms covered by
`TOPAL-COMPILER-LEXICAL-RETURN-INFIX-COLLECT-001`, qualified, nested-source,
decision, callback, cleanup-bearing, and other collection forms SHALL remain
fail-closed under their existing diagnostic contracts. This rule SHALL
introduce no runtime control-flow object, collection materialization,
comparison, allocation, indirect call, unwind edge, foreign
dependency, C/C++ standard library, public ABI, or native-ABI revision.
...
compiler model SHALL retain the returning block as the function result, and the
backend SHALL preserve its nested DWARF scope before using the enclosing
function's existing single machine return. Except for the infix Array and String
forms covered by `TOPAL-COMPILER-LEXICAL-RETURN-INFIX-COLLECT-001`, invalid
policies, qualified, nested-source, decision, callback, cleanup-bearing, and
other collection forms SHALL remain fail-closed under their existing diagnostic
...
### TOPAL-COMPILER-LEXICAL-RETURN-INFIX-COLLECT-001 — Infix collection-source lexical exit

An admitted cleanup-free lexical block used as the direct left source of the
exact built-in `source collect Array` or `source collect String` operation MAY
execute an explicit `return`. Selection of the exact operation and target SHALL
precede the source. The source return SHALL then complete the nearest enclosing
ordinary function before finite-List classification, Array extent derivation,
String-entry validation, or target materialization. No Array or String
collection result, consumption action, or abandoned binding SHALL be emitted,
and remaining source statements and the enclosing function tail SHALL be
excluded from generated IR at `-O0`.

The returned value SHALL be validated against the enclosing function result
classifier rather than the collection target or abandoned binding classifier.
The checked compiler model SHALL retain the returning block as the function
result, and the backend SHALL preserve its nested DWARF scope before using the
enclosing function's existing single machine return. Unknown or qualified
targets, nested-source, decision, callback, cleanup-bearing, and other
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete
List decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, callbacks,
...
### TOPAL-COMPILER-LEXICAL-RETURN-EXHAUSTIVE-BOOLEAN-SUBJECT-001 — Exhaustive Boolean-decision subject lexical exit

An admitted cleanup-free lexical block used as the direct subject of a complete
Boolean decision consisting of exactly one `false` rule and one `true` rule,
in either source order and without `otherwise`, MAY execute an explicit
`return`. Recognition of the exact two-rule matcher set SHALL precede the
subject. The subject return SHALL then complete the nearest enclosing ordinary
function before Boolean classification, matcher consideration, action
selection, or action evaluation. No decision branch, selected action,
...
### TOPAL-COMPILER-LEXICAL-RETURN-COMPARISON-DECISION-SUBJECT-001 — Comparison-decision subject lexical exit

An admitted cleanup-free lexical block used as the direct subject of a complete
ordered comparison decision consisting of one or more comparison rules followed
by a final `otherwise` rule MAY execute an explicit `return`. Recognition of
that exact matcher sequence SHALL precede the subject. The subject return SHALL
then complete the nearest enclosing ordinary function before exact-number
classification, matcher-operand evaluation, comparison, action selection, or
action evaluation. No comparison operation, decision branch, selected action,
abandoned decision value, or following function tail SHALL be emitted at
`-O0`.
...
decision branch, allocation, indirect call, unwind edge, foreign dependency,
C/C++ standard library, public ABI, or native-ABI revision.

### TOPAL-COMPILER-LEXICAL-RETURN-BOOLEAN-DECISION-ACTIONS-001 — Complete Boolean action lexical exits

Every action of an admitted complete Boolean decision MAY be a cleanup-free
lexical block that directly executes an explicit `return`. The decision SHALL
first evaluate its Boolean subject once and select exactly one action in source
order. The selected action return SHALL complete the nearest enclosing ordinary
...
When every action returns, the checked compiler model SHALL retain the complete
Boolean decision as the function result with each returning block normalized to
its returned value. LLVM lowering SHALL retain the ordinary Boolean branch and
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete
List decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, a nested
...
### TOPAL-COMPILER-LEXICAL-RETURN-COMPARISON-VALUE-DECISION-ACTIONS-001 — Complete Comparison action lexical exits

Every action of an admitted complete decision over the closed `Comparison`
value MAY be a cleanup-free lexical block that directly executes an explicit
`return`. The decision SHALL evaluate its subject once and select exactly one
`Less`, `Equal`, `Greater`, or final-fallback action in source order. The
selected action return SHALL complete the nearest enclosing ordinary function,
validate its value against that function's result classifier, and exclude the
remaining action statements and following function tail from generated IR at
`-O0`.

When every action returns, the checked compiler model SHALL retain the complete
Comparison-value decision as the function result with each action block
...
outside `TOPAL-COMPILER-LEXICAL-RETURN-ENUM-FALLBACK-DECISION-ACTIONS-001` and
`TOPAL-COMPILER-LEXICAL-RETURN-ENUM-EXHAUSTIVE-DECISION-ACTIONS-001`, and
complete Optional decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-OPTIONAL-DECISION-ACTIONS-001`, and
complete Result decisions whose every action returns as admitted by
...
### TOPAL-COMPILER-LEXICAL-RETURN-ENUM-FALLBACK-DECISION-ACTIONS-001 — Final-fallback Enum action lexical exits

Every action of an admitted decision over an already-declared non-Comparison
Enum MAY be a cleanup-free lexical block that directly executes an explicit
`return` when the decision has a nonempty prefix of distinct identifier
matchers followed by final `otherwise`. The decision SHALL evaluate its Enum
subject once, consider alternatives in source order, and evaluate only the
selected action. The selected action return SHALL complete the nearest enclosing
ordinary function, validate its value against that function's result
classifier, and exclude the remaining action statements and following function
tail from generated IR at `-O0`.

When every action returns, the checked compiler model SHALL retain the Enum
decision as the function result with each action block normalized to its
returned value. LLVM lowering SHALL retain the ordinary tag switch and typed SSA
join, preserve each action block's nested DWARF scope, and use the enclosing
function's existing single machine return. Duplicate or unknown alternatives,
an empty prefix, a nonfinal or missing fallback, an exhaustive no-fallback
table outside
`TOPAL-COMPILER-LEXICAL-RETURN-ENUM-EXHAUSTIVE-DECISION-ACTIONS-001`, and
complete Optional decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-OPTIONAL-DECISION-ACTIONS-001`, and
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete
List decisions whose every action returns as admitted by
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, another
...
introduce no runtime control-flow object, allocation, indirect call, unwind edge,
foreign dependency, C/C++ standard library, public ABI, or native-ABI revision.

### TOPAL-COMPILER-LEXICAL-RETURN-ENUM-EXHAUSTIVE-DECISION-ACTIONS-001 — Exhaustive Enum action lexical exits

Every action of an admitted exhaustive decision over an already-declared
non-Comparison Enum MAY be a cleanup-free lexical block that directly executes
an explicit `return`. The decision SHALL contain a nonempty set of distinct
identifier matchers equal to the Enum's declared alternative set and SHALL NOT
contain `otherwise`. It SHALL evaluate its Enum subject once, consider
alternatives in source order, and evaluate only the selected action. The
selected action return SHALL complete the nearest enclosing ordinary function,
validate its value against that function's result classifier, and exclude the
remaining action statements and following function tail from generated IR at
`-O0`.

When every action returns, the checked compiler model SHALL retain the
exhaustive Enum decision as the function result with each action block
normalized to its returned value. LLVM lowering SHALL retain the ordinary tag
switch and typed SSA join, preserve each action block's nested DWARF scope, and
use the enclosing function's existing single machine return. Missing, foreign,
unknown, or duplicate alternatives, fallback tables outside
`TOPAL-COMPILER-LEXICAL-RETURN-ENUM-FALLBACK-DECISION-ACTIONS-001`, another
decision classifier, any normally completing action, a nested action
expression, callbacks, generators, and cleanup-bearing actions SHALL remain
fail-closed until their separate exit-edge and cleanup lowering is admitted.
This rule SHALL introduce no runtime control-flow object, allocation, indirect
call, unwind edge, foreign dependency, C/C++ standard library, public ABI, or
native-ABI revision.

### TOPAL-COMPILER-LEXICAL-RETURN-OPTIONAL-DECISION-ACTIONS-001 — Complete Optional action lexical exits

Every action of an admitted complete decision over `Optional T` MAY be a
cleanup-free lexical block that directly executes an explicit `return`. The
decision SHALL contain distinct `Some name` and `None` matchers in either order,
one of those matchers followed by final `otherwise`, or only `otherwise`.
It SHALL evaluate its Optional subject once, consider rules in source order,
bind a present payload only when an explicit `Some name` action is selected,
and evaluate only the selected action. The selected action return SHALL complete
the nearest enclosing ordinary function, validate its value against that
function's result classifier, and exclude the remaining action statements and
following function tail from generated IR at `-O0`.

When every action returns, the checked compiler model SHALL retain the complete
Optional decision as the function result with each action block normalized to
its returned value. LLVM lowering SHALL retain the ordinary Optional tag query,
two-way branch, conditional payload extraction, and typed SSA join, preserve
each action block's nested DWARF scope, and use the enclosing function's existing
...
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, a complete List
decision outside `TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, or another
...
### TOPAL-COMPILER-LEXICAL-RETURN-RESULT-DECISION-ACTIONS-001 — Exhaustive Result action lexical exits

Every action of an admitted exhaustive decision over `Result (T, Codes)` MAY be
a cleanup-free lexical block that directly executes an explicit `return`. The
decision SHALL contain exactly one `Ok value` and one `Error problem` matcher in
either source order. It SHALL evaluate its Result subject once, consider rules
in source order, bind the selected success value or structured Error only in its
action, and evaluate only that action. The selected action return SHALL complete
the nearest enclosing ordinary function, validate its value against that
function's result classifier, and exclude the remaining action statements and
following function tail from generated IR at `-O0`.

When both actions return, the checked compiler model SHALL retain the exhaustive
Result decision as the function result with each action block normalized to its
...
### TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001 — Complete List action lexical exits

Every action of an admitted complete decision over `List T` MAY be a
cleanup-free lexical block that directly executes an explicit `return`. The
decision SHALL contain exactly one `Entry (first, rest)` matcher and one
`Empty` matcher in either order, with distinct entry binding names. It SHALL
evaluate its List subject once, select the matching rule in source order, bind
the first entry and remaining List only when the Entry action is selected, and
evaluate only the selected action.

The selected action return SHALL complete the nearest enclosing ordinary
function, validate its value against that function's result classifier, and
exclude the remaining action statements and following function tail from
generated IR at `-O0`. The checked compiler model SHALL retain the complete
List decision, its entry bindings, exact supported element facts, and both
normalized actions as the function result. LLVM lowering SHALL retain the
ordinary empty test, conditional head/rest decomposition, two action edges, a
typed SSA join, nested action and binding DWARF scopes, and the enclosing
function's existing single machine return.

Duplicate binding names, missing, duplicate, mixed, or additional matchers; a
non-List subject; an unsupported List element representation; any normally
completing action; nested action expressions; callbacks; generators; and
cleanup-bearing actions SHALL remain fail-closed. This rule SHALL introduce no
runtime control-flow object, allocation beyond the subject's existing List
representation, indirect call, unwind edge, foreign dependency, C/C++ standard
library, public ABI, or native-ABI revision.
