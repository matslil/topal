...
count-accessible is fn (state : (Int, List String), point : (Int, Int)) -> (Int, List String)
  ...
  increment is fn (accepted : Boolean, count : Int) -> Int
    accepted
      true then count + 1
      false then count
  next-count is increment (accessible (occupied, point), count-of state)
  (next-count, occupied)

solve is fn (input : String) -> Int
  points is points-from input
  keys is points fold (Empty String) { selected, point } selected append (point-key point)
  occupied is keys
  final is points fold (0, occupied) { state, point } count-accessible (state, point)
