...
outside 3b2-b5e8q/3b2-b5e8r/3b2-b5e8s/3b2-b5e8t/3b2-b5e8u/3b2-b5e8v/3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad,
...
other matcher sets, nested subjects, decision-action returns outside 3b2-b5e8v/3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad,
conditional control flow, and cleanup-bearing forms remain fail-closed.
...
decision-action returns outside 3b2-b5e8v/3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, conditional control flow, and
cleanup-bearing forms remain fail-closed.
...
nested subjects, decision-action returns outside 3b2-b5e8v/3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, conditional control
...
subjects, decision-action returns outside 3b2-b5e8v/3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, conditional control flow,
and cleanup-bearing forms remain fail-closed.
...
3b2-b5e8v/3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, conditional control flow, and cleanup-bearing forms remain
fail-closed.

Increment 3b2-b5e8v admits a complete Boolean decision when every action is a
direct cleanup-free returning block. One shared frontend predicate selects the
exact complete shape for both execution paths. The interpreter selects the
action before entering it through the function-aware resumable block path. The
checked model normalizes each branch block to its returned value and retains
the complete decision as the function result, so the backend emits the ordinary
Boolean branch, a typed SSA phi join, and the existing single machine return.
...
classifiers outside 3b2-b5e8w/3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, nested action expressions,
...
predicate recognizes distinct `Less`/`Equal`/`Greater` matchers with complete
coverage or a final fallback and requires every action to return. The
interpreter retains source-order selection through the function-aware block
path. The checked model normalizes the three action blocks into the existing
Comparison-value decision, and LLVM emits its ordinary switch, typed SSA phi,
...
3b2-b5e8x/3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, nested action expressions, callbacks, and
cleanup-bearing actions remain fail-closed.
...
The shared frontend predicate establishes that every action exits before either
execution path enters the decision. The interpreter evaluates the subject once,
then retains source-order matcher-operand evaluation and selection before using
the function-aware block path for the selected action. The checked model
normalizes each action block into the existing ordered-comparison decision, and
LLVM emits its ordinary comparison/branch chain, typed SSA phi, and single
...
mixed actions, other decision classifiers outside 3b2-b5e8y/3b2-b5e8z/3b2-b5e8aa/3b2-b5e8ab/3b2-b5e8ac/3b2-b5e8ad, nested
action expressions, callbacks, and cleanup-bearing actions remain fail-closed.
...
fallbacks, exhaustive no-fallback tables outside 3b2-b5e8z, mixed actions,
complete Optional decisions outside 3b2-b5e8aa, complete ordinary Result
decisions outside 3b2-b5e8ab, qualified Error-code Result decisions outside
3b2-b5e8ac/3b2-b5e8ad, other decision classifiers, nested action expressions, callbacks,
and cleanup-bearing actions remain fail-closed.

Increment 3b2-b5e8z admits all-returning actions for an exhaustive
already-declared non-Comparison Enum decision without `otherwise`. A shared
frontend predicate establishes a nonempty distinct-identifier matcher set
before subject evaluation; the evaluated nominal Enum identity then proves the
set equal to the declaration's complete alternative set. The interpreter
retains source-order selection before using the function-aware block path for
the selected action. The checked model validates alternatives before
return-aware action analysis, normalizes every action block into the existing
Enum decision, and LLVM emits its ordinary tag switch, typed SSA phi, and
single machine return while preserving all nested action debug scopes. The
subject-based distinction also keeps user-declared alternatives named `Less`,
`Equal`, or `Greater` separate from built-in `Comparison`. No runtime
control-flow carrier, allocation, unwind edge, foreign dependency, or ABI
change is needed; missing, foreign, unknown, or duplicate alternatives,
fallback tables outside 3b2-b5e8y, mixed actions, other decision classifiers,
nested action expressions, callbacks, and cleanup-bearing actions remain
fail-closed except for complete Optional decisions admitted by 3b2-b5e8aa and
complete ordinary Result decisions admitted by 3b2-b5e8ab and qualified
Error-code Result decisions admitted by 3b2-b5e8ac.
Complete List decisions are admitted by 3b2-b5e8ad.

Increment 3b2-b5e8aa admits all-returning actions for complete Optional
decisions. One shared frontend predicate recognizes distinct `Some name` and
`None` rules in either order, either explicit alternative followed by final
`otherwise`, or `otherwise` alone, and requires every action to return. The
interpreter evaluates the subject once, retains source-order selection, binds a
present payload only for a selected explicit `Some name`, and enters only that
action through the function-aware block path. The checked model retains the
same payload facts in the Some branch, validates every returned value against
the enclosing function result, and normalizes both actions into the existing
Optional decision. LLVM emits the ordinary Optional tag query, branch,
conditional payload extraction, typed SSA phi, and single machine return while
preserving both nested action debug scopes and the payload binding. The
otherwise-only form is classified from the evaluated or checked subject rather
than being assigned Boolean meaning. No runtime control-flow carrier,
allocation, unwind edge, foreign dependency, other-language standard library,
or ABI change is needed; duplicate, incomplete, mixed-matcher, nonfinal
fallback, non-Optional, mixed-action, nested-action, callback, and
cleanup-bearing forms remain fail-closed, as do complete Result decisions
outside 3b2-b5e8ab, qualified Error-code Result decisions outside 3b2-b5e8ac,
and complete List decisions outside 3b2-b5e8ad.

Increment 3b2-b5e8ab admits all-returning actions for the exhaustive ordinary
Result matcher pair. One shared frontend predicate recognizes exactly one `Ok
value` and one `Error problem` in either source order and requires both actions
to return. The interpreter evaluates the subject once, retains source-order
selection, binds the selected success value or structured Error only in its
action, and enters that action through the function-aware block path. The
checked model transfers exact success facts into the Ok branch, preserves Error
identity in the other branch, validates both returned values against the
enclosing function result, and normalizes the actions into the existing Result
decision. LLVM emits the ordinary Result tag query, branch, path-local payload
extraction, typed SSA phi, and single machine return while preserving both
payload bindings and nested action debug scopes. No runtime control-flow
carrier, allocation, unwind edge, foreign dependency, other-language standard
library, or ABI change is needed; qualified Error-code tables outside
3b2-b5e8ac, complete List decisions outside 3b2-b5e8ad, duplicate, missing,
mixed-matcher, non-Result, mixed-action,
nested-action, callback, and cleanup-bearing forms remain fail-closed.

Increment 3b2-b5e8ac admits all-returning actions for complete qualified
arithmetic Error-code Result decisions. A shared frontend predicate requires
exactly one `Ok value`, at least one qualified arithmetic code, and either a
later whole-`Error problem` fallback or exact coverage of all four closed codes;
every action must return directly. The interpreter evaluates the subject once,
retains source-order consideration and nominal code matching, binds only a
selected success or whole-Error payload, and enters only the selected action
through the function-aware block path. The checked model retains exact success
facts, ordered nominal code identities, the optional fallback, and normalized
branch values. LLVM reuses the ordinary Result tag branch and payload
extraction, emits one code query and switch, retains the fail-closed unreachable
default for exact no-fallback coverage, joins all selected values through a
typed SSA phi, and uses the existing single machine return. Every action keeps
its nested `DILexicalBlock`; selected Ok and whole-Error bindings keep their
source-level DWARF identity. No runtime control-flow carrier, allocation, unwind
edge, foreign dependency, other-language standard library, or ABI change is
needed. Unknown, duplicate, incomplete, post-fallback, mixed-matcher,
non-arithmetic or multi-vocabulary, non-Result, mixed-action, nested-action,
callback, and cleanup-bearing forms remain fail-closed.

Increment 3b2-b5e8ad admits all-returning actions for complete List decisions.
A shared frontend predicate requires exactly one `Entry (first, rest)` matcher,
one `Empty` matcher, distinct entry binding names, and two directly returning
actions. The interpreter evaluates the subject once, preserves source-order
selection, decomposes only a selected nonempty List, and enters only the
selected action through the function-aware block path. The checked model
retains supported element facts, the path-local entry bindings, and normalized
branch values. LLVM reuses the ordinary List empty test and conditional
head/rest decomposition, joins both action values through a typed SSA phi, and
uses the existing single machine return. Both actions and the selected Entry
bindings keep their nested DWARF identity. No runtime control-flow carrier,
allocation beyond the existing List representation, unwind edge, foreign
dependency, other-language standard library, or ABI change is needed.
Duplicate bindings, incomplete or mixed matcher sets, non-List subjects,
unsupported element representations, mixed-action, nested-action, callback,
and cleanup-bearing forms remain fail-closed.
