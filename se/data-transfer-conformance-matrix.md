...
| 17. Firewall | `TOPAL-DATA-OFFLOAD-001` | self-checking `examples/data-transfer/firewall.t` introduces views; `examples/data-transfer/packet-filter.t` keeps its single-consumer policy local while demonstrating fail-closed validation, immutable policy, exact-flow priority, IPv4/IPv6 policy equivalence, batching, and slow-path decisions | native region handoff, software/offload differential, and resource measurements remain | partial |
