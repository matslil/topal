...
| 1b | canonical `lint` language-feature retention and the empty authority-free `lang lint` Scope with exact display and debugging | complete |
...
| 3b2-b5ac | exact repeated anonymous-pattern identity for capture-free Function-containing Tuple/Record values, structural i32 observation-field guards, and single aggregate debugging | complete |
...
| 3b2-b5aj | closed scalar packages in either or both syntactic operand positions, global source-order retention, operand/field declaration-order flattening, exact private calls, and field debugging | complete |
| 3b2-b5ak | exact capture-free Tuple and Record package fields, source-order retention, structured closed defaults, exact aggregate private parameters, and structured field debugging | complete |
...
| 3b2-b5aq | exact root/root-alias Scope package fields, source-order namespace-fact retention, closed root defaults, existing specialized hidden-data transport, and Scope/data debugging | complete |
...
| 3b2-b5e8d | cleanup-free lexical blocks that return from a direct positional or labeled product field, with ordered once-only preceding-field evaluation, omitted aggregate construction and tails, and nested DWARF | complete |
| 3b2-b5e8e | cleanup-free lexical blocks that return from a direct unary or binary argument of one exact flat named declaration, with ordered once-only left-prefix evaluation, omitted callee invocation and tails, and nested DWARF | complete |
| 3b2-b5e8f | cleanup-free lexical blocks that return from the direct payload of the built-in unary `Some` constructor, with omitted payload classification, Optional construction, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8g | cleanup-free lexical blocks that return from direct strict unary `String`/`Int`/`Nat`/`Rational` or declared payload-Union construction, with omitted conversion, classification, construction, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8w | complete Comparison-value decisions whose every action is a direct cleanup-free returning block, with once-only subject evaluation and source-order selection, normalized branch values, LLVM switch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8x | complete ordered exact-numeric comparison decisions whose every action is a direct cleanup-free returning block, with once-only subject and source-order matcher evaluation, normalized branch values, LLVM comparison chain/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8y | final-fallback declared-Enum decisions whose every action is a direct cleanup-free returning block, with once-only subject evaluation and source-order selection, normalized branch values, LLVM tag switch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8z | exhaustive declared-Enum decisions without fallback whose every action is a direct cleanup-free returning block, with nominal alternative-set validation, once-only subject evaluation and source-order selection, normalized branch values, LLVM tag switch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8aa | complete Optional decisions whose every action is a direct cleanup-free returning block, with once-only subject evaluation, source-order selection, selected Some-payload binding, normalized branch values, LLVM Optional branch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8ab | exhaustive Result decisions whose Ok and Error actions are direct cleanup-free returning blocks, with once-only subject evaluation, source-order selection, path-local success/Error binding, normalized branch values, LLVM Result branch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8ac | complete qualified arithmetic Error-code Result decisions whose every action is a direct cleanup-free returning block, with once-only subject evaluation, ordered nominal code selection, selected success/Error binding, normalized branch values, LLVM Result branch/code switch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8ad | complete List decisions whose Entry and Empty actions are direct cleanup-free returning blocks, with once-only subject evaluation, source-order selection, path-local head/rest binding, normalized branch values, LLVM empty-test/decomposition/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8v | complete Boolean decisions whose every action is a direct cleanup-free returning block, with once-only subject evaluation and source-order selection, normalized branch values, LLVM branch/typed phi/single return, skipped tails, and nested action DWARF | complete |
| 3b2-b5e8u | cleanup-free lexical blocks that return from the direct subject of every parser-proven complete decision, using syntax-node construction as prior evidence and omitting subject classification, type-specific exhaustiveness/matcher/operand/binding work, action selection/evaluation, discarded decision value, and tails plus nested DWARF | complete |
| 3b2-b5e8t | cleanup-free lexical blocks that return from the direct subject of any parser-accepted decision ending in final `otherwise`, after fallback recognition and with omitted subject classification, matcher compatibility/operand/binding work, action selection/evaluation, discarded decision value, and tails plus nested DWARF | complete |
| 3b2-b5e8s | cleanup-free lexical blocks that return from the direct subject of a complete ordered comparison decision with one or more comparison matchers and final `otherwise`, after exact matcher-sequence recognition and with omitted subject classification, matcher-operand evaluation, comparison, action selection/evaluation, discarded decision value, and tails plus nested DWARF | complete |
| 3b2-b5e8r | cleanup-free lexical blocks that return from the direct subject of a complete Boolean decision with exactly one `false` and one `true` rule in either order, after exact matcher-set recognition and with omitted classification, matching, action selection/evaluation, discarded decision value, and tails plus nested DWARF | complete |
| 3b2-b5e8q | cleanup-free lexical blocks that return from the direct subject of a complete Boolean decision with one literal rule followed by `otherwise`, after exact shape recognition and with omitted classification, matching, action selection/evaluation, discarded decision value, and tails plus nested DWARF | complete |
| 3b2-b5e8p | cleanup-free lexical blocks that return from the direct left source of exact built-in `source collect Array` or `source collect String`, after operation/target selection and with omitted classification, extent/entry validation, materialization, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8o | cleanup-free lexical blocks that return from the direct source of exact built-in `collect-map source resolving policy`, after clause/policy validation and with omitted pair classification, equality/collision work, materialization, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8n | cleanup-free lexical blocks that return from the direct source of exact built-in `collect-set` or `collect-bag`, after operation selection and with omitted classification, coalescing/multiplicity work, materialization, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8m | cleanup-free lexical blocks that return from the direct source of built-in unary `collect`, after exact operation selection and with omitted traversal classification, consumption, List materialization, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8l | cleanup-free lexical blocks that return from the direct left operand of `operand modulo Name` for an already-declared named modular target, after identity/declaration-order validation and with omitted reduction, nominal construction, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8k | cleanup-free lexical blocks that return from the direct operand of an already-declared named `ModNat` or `ModInt` type, after identity/declaration-order validation and with omitted range validation, Result/nominal construction, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8j | cleanup-free lexical blocks that return from the direct operand of an already-declared named Int constraint, after identity/base/declaration-order validation and with omitted predicate, evidence, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8i | cleanup-free lexical blocks that return from the direct operand of built-in `Character`, after constructor selection and with omitted constraint validation, evidence, binding, and tails plus nested DWARF | complete |
| 3b2-b5e8h | cleanup-free lexical blocks that return from a valid direct payload of an already-declared positional Variant, after identity/index validation and with omitted classification, construction, binding, and tails plus nested DWARF | complete |
...
| 4b3d-ah | ordinary immutable List (String, String) private parameter/result/package and nested Tuple/Record passage, exact fieldwise equality, complete decomposition, count, emptiness, display, and debugging | complete |
| 4b3d-ag | ordinary immutable List (String, Int) private parameter/result/package and nested Tuple/Record passage, exact fieldwise equality, complete decomposition, count, emptiness, display, and debugging | complete |
...
| 6b2b2g | exact local retained named Function alias chains and non-escaping nested calls with selected live-root capture sets, shadow isolation, direct lowering, and GDB observation | complete |
| 6b2b2h | exact private named-overload, anonymous, and non-escaping nested Function values carrying live-root environments through scalar and Tuple/Record parameters/results, with direct selected calls and GDB observation | complete |
...
| 6b2b2m | exact finite List Function entries carrying immutable live-root snapshots through private parameters/results, decisions, packages, Tuple/Record containment, and GDB observation | complete |
| 6b2b2n | exact fixed-size Array Function entries carrying immutable live-root snapshots through private parameters/results, checked access, packages, Tuple/Record containment, and GDB observation | complete |
...
| 6c2e | exact local retained named Function alias chains and non-escaping nested calls with selected defining-context capture sets, shadow isolation, direct lowering, and GDB observation | complete |
| 6c2f | exact private named-overload, anonymous, and non-escaping nested Function values carrying defining-context environments through scalar and Tuple/Record parameters/results, with direct selected calls and GDB observation | complete |
...
| 6c2k | exact finite List Function entries carrying immutable defining-context snapshots through private parameters/results, decisions, packages, Tuple/Record containment, and GDB observation | complete |
| 6c2l | exact fixed-size Array Function entries carrying immutable defining-context snapshots through private parameters/results, checked access, packages, Tuple/Record containment, and GDB observation | complete |
| 6c2m | exact nonempty Map (String, Function) values carrying immutable defining-context snapshots through private parameters/results, exact-key lookup, collision policies, packages, Tuple/Record containment, and GDB observation | complete |
| 6c2 | recursive/overloaded nested Function environments beyond 6c2m; escaping anonymous Functions and nested Functions outside 6c2g; unsupported Function-containing representations; other callable environments; qualified root access beyond 6b2b2o; and public/library context ABI | planned |
...
publication. Increment 1b retains the canonical `lint` variant identity in the
checked program and exposes its empty `lang lint` namespace as a private Scope
alternative without importing lint execution or host authority. Future library
interfaces must record their language revision, feature set, vocabulary, and
authority independently of this representation. Increment 2a removes the
integer boundary with a freestanding Topal
...
closed contextual Int/Nat infinity identity and ordering subset; 2c-c2 adds
...
representations; 3b2-b5w threads already-evaluated immutable capture
environments through specialized private Function parameters and direct
anonymous or nested calls; 3b2-b5x returns the same immutable capture facts and
...
Tuple/Record values while keeping observation tags non-dispatching; 3b2-b5ad
extends scalar repeated identity to same-source captured anonymous Functions by
guarding their ordered represented capture snapshots after the canonical source
identity; 3b2-b5ae composes both mechanisms for captured Function leaves in
...
defaults while keeping the existing flat private call boundary; 3b2-b5aj
extends that boundary to package/package and mixed package/scalar calls with
global explicit-value ordering and operand/field declaration-order flattening;
3b2-b5ak admits exact capture-free Tuple and Record fields without decomposing
their private aggregate parameters into additional package fields; 3b2-b5al
does the same for exact nominal Sum tag-plus-payload parameters; 3b2-b5am
retains exact direct Function identity and capture facts through package-order
...
boundary; 3b2-b5aq retains exact root/root-alias namespace snapshots through
package ordering while reusing the specialized Scope tag and hidden represented
data boundary; 3b2-b5ar returns exact nonrecursive/nonoverloaded nested
declarations with their immutable lexical/context/root snapshots through scalar
and Tuple/Record private results while preserving direct application; 3b2-b5as
extends the same exact callable and capture facts through present/absent
...
dynamic Error propagation; 3b2-b5av extends them through exact finite List
entries, source-index capture paths, packages, recursive products, and complete
...
distinct capture sets and rejects value-fact-dependent choices; 6b2b2g/6c2e
follows exact local named Function aliases and non-escaping nested calls while
keeping context/root values out of duplicate lexical-capture groups;
6b2b2h/6c2f carries exact named-overload, anonymous, and non-escaping nested
Function environments through private scalar and Tuple/Record parameters and
results, retains per-declaration selection separately from the boundary
environment, and preserves active and suspended frames without indirect calls;
6b2b2i/6c2g permits one exact nonrecursive/nonoverloaded nested declaration to
escape those private result boundaries with its immutable live-root and
defining-context snapshots while retaining direct application;
...
Error propagation; 6b2b2m/6c2k carries them through exact finite List entry
paths, packages, Tuple/Record containment, and complete List decisions;
6b2b2n/6c2l carries them through fixed-size Array entry paths, packages,
Tuple/Record containment, and exact checked access; and
