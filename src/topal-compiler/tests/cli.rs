...
fn repeated_function_aggregate_values_are_exact_freestanding_and_debuggable() {
    ...
    let capturing_source = directory.join("capturing-function-aggregate-pattern.t");
    fs::write(
        &capturing_source,
        "use language (version is v0.1)\noffset is 1\ncaptured : Function is { value } value + offset\npackage is (operation is captured, value is 41)\nrepeat : Function is { value, value } 42\nrepeat (package, package)\n",
    )
    .unwrap();
    let capturing = run(topalc().args([
        "-o",
        directory.join("capturing").to_str().unwrap(),
        capturing_source.to_str().unwrap(),
    ]));
    assert!(!capturing.status.success());
    let diagnostic = String::from_utf8_lossy(&capturing.stderr);
    assert!(
        diagnostic.contains("E-COMPILER-UNSUPPORTED"),
        "{diagnostic}"
    );
    assert!(
        diagnostic.contains("capture-free callable facts"),
        "{diagnostic}"
    );
