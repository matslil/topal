    ...
    def to_string(self):
        ...
        while address:
            ...
            if self._element_type == "Effect":
                ...
                if rendered is None:
                    return f"<invalid List Type entry {payload}>"
                entries.append(rendered)
                ...
                entries.append(rendered)
            elif self._element_type == "Optional Rational":
                payload = int.from_bytes(node[0:8], "little")
                if not payload:
                    return "<invalid null List Optional Rational entry>"
                rendered = _TopalOptionalPrinter(payload, "Rational").to_string()
                if rendered.startswith("<"):
                    return f"<invalid List Optional Rational entry: {rendered}>"
                entries.append(rendered)
            elif self._element_type == "Optional String":
                payload = int.from_bytes(node[0:8], "little")
                if not payload:
                    return "<invalid null List Optional String entry>"
                rendered = _TopalOptionalPrinter(payload, "String").to_string()
                if rendered.startswith("<"):
                    return f"<invalid List Optional String entry: {rendered}>"
                entries.append(rendered)
