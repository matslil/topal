...
define internal i1 @topal.runtime.list.completed.equal(ptr %left, ptr %right) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %left.next.pointer = getelementptr %topal.ListCompletedStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListCompletedStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.completed.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListCompletedStorage, ptr %current, i32 0, i32 1
