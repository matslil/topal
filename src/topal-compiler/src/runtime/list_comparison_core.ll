...
define internal i1 @topal.runtime.list.comparison.equal(ptr %left, ptr %right) nounwind noinline {
entry:
  br label %loop
loop:
...
compare:
  %left.value.pointer = getelementptr %topal.ListComparisonStorage, ptr %left.current, i32 0, i32 0
  %right.value.pointer = getelementptr %topal.ListComparisonStorage, ptr %right.current, i32 0, i32 0
  %left.value = load i32, ptr %left.value.pointer, align 4
  %right.value = load i32, ptr %right.value.pointer, align 4
  %equal = icmp eq i32 %left.value, %right.value
  br i1 %equal, label %advance, label %different
advance:
  %left.next.pointer = getelementptr %topal.ListComparisonStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListComparisonStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.comparison.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListComparisonStorage, ptr %current, i32 0, i32 1
