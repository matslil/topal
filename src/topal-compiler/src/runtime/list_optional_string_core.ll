...
define internal i1 @topal.runtime.list.optional.string.equal(ptr %left, ptr %right) nounwind noinline {
entry:
  br label %loop
loop:
...
compare:
  %left.value.pointer = getelementptr %topal.ListOptionalStringStorage, ptr %left.current, i32 0, i32 0
  %right.value.pointer = getelementptr %topal.ListOptionalStringStorage, ptr %right.current, i32 0, i32 0
  %left.value = load ptr, ptr %left.value.pointer, align 8
  %right.value = load ptr, ptr %right.value.pointer, align 8
  %equal = call i1 @topal.runtime.optional.string.equal(ptr %left.value, ptr %right.value)
  br i1 %equal, label %advance, label %different
advance:
  %left.next.pointer = getelementptr %topal.ListOptionalStringStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListOptionalStringStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.optional.string.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListOptionalStringStorage, ptr %current, i32 0, i32 1
