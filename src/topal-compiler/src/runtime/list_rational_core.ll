...
compare:
  %left.value.pointer = getelementptr %topal.ListRationalStorage, ptr %left.current, i32 0, i32 0
  %right.value.pointer = getelementptr %topal.ListRationalStorage, ptr %right.current, i32 0, i32 0
  %left.value = load ptr, ptr %left.value.pointer, align 8
  %right.value = load ptr, ptr %right.value.pointer, align 8
  %ordering = call i32 @topal.runtime.rational.compare(ptr %left.value, ptr %right.value)
  %equal = icmp eq i32 %ordering, 0
  br i1 %equal, label %advance, label %different
advance:
  %left.next.pointer = getelementptr %topal.ListRationalStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListRationalStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.rational.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListRationalStorage, ptr %current, i32 0, i32 1
