...
define internal i1 @topal.runtime.list.type.equal(ptr %left, ptr %right) nounwind noinline {
entry:
  br label %loop
loop:
...
compare:
  %left.value.pointer = getelementptr %topal.ListTypeStorage, ptr %left.current, i32 0, i32 0
  %right.value.pointer = getelementptr %topal.ListTypeStorage, ptr %right.current, i32 0, i32 0
...
advance:
  %left.next.pointer = getelementptr %topal.ListTypeStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListTypeStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.type.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListTypeStorage, ptr %current, i32 0, i32 1
