...
define internal i1 @topal.runtime.list.enum.equal(ptr %left, ptr %right) nounwind noinline {
entry:
  br label %loop
loop:
...
compare:
  %left.value.pointer = getelementptr %topal.ListEnumStorage, ptr %left.current, i32 0, i32 0
  %right.value.pointer = getelementptr %topal.ListEnumStorage, ptr %right.current, i32 0, i32 0
...
advance:
  %left.next.pointer = getelementptr %topal.ListEnumStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListEnumStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.enum.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListEnumStorage, ptr %current, i32 0, i32 1
