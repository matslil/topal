...
define internal i1 @topal.runtime.list.effect.equal(ptr %left, ptr %right) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %left.next.pointer = getelementptr %topal.ListEffectStorage, ptr %left.current, i32 0, i32 1
  %right.next.pointer = getelementptr %topal.ListEffectStorage, ptr %right.current, i32 0, i32 1
...
define internal ptr @topal.runtime.list.effect.entry.count(ptr %list) nounwind noinline {
entry:
  br label %loop
loop:
...
advance:
  %next.pointer = getelementptr %topal.ListEffectStorage, ptr %current, i32 0, i32 1
