...
admitted by `TOPAL-COMP-LEXICAL-RETURN-MODULAR-REDUCE-001`, and the direct unary
...
`TOPAL-COMP-LEXICAL-RETURN-BOOLEAN-DECISION-ACTIONS-001`, and complete
Comparison-value decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-COMPARISON-VALUE-DECISION-ACTIONS-001`, and complete
...
`TOPAL-COMP-LEXICAL-RETURN-ENUM-EXHAUSTIVE-DECISION-ACTIONS-001`, and
complete Optional decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-OPTIONAL-DECISION-ACTIONS-001`, and
complete Result decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-RESULT-DECISION-ACTIONS-001`, and qualified
Error-code Result decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete List
decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, a return-bearing
...
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete List
decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, the implementation
...
## TOPAL-COMP-LEXICAL-RETURN-OPERATOR-001 — Symbolic-operand lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as a direct left or
right operand of a symbolic operator. A left exit shall omit the operator and
right operand. A right exit shall retain the complete left application prefix
exactly once in source order, abandon its value, and omit the operator. Both
forms shall validate the returned value against the function result classifier
and omit the lexical and function tails at O0.

The compiler shall represent an evaluated left prefix and returned lexical
block as a private exit sequence, emit the prefix in its enclosing debug scope,
retain the block's nested DWARF scope, and use the existing private result and
single machine return. Constructors other than the direct `Some` payload
covered by `TOPAL-COMP-LEXICAL-RETURN-OPTIONAL-001` and the strict unary
constructors covered by `TOPAL-COMP-LEXICAL-RETURN-CONSTRUCTOR-001`, packaged or
nested call arguments, overloaded or indirect calls, decisions, callbacks,
generators, and scopes with cleanup obligations shall remain fail-closed.
The implementation shall add no runtime control-flow value, unwind edge,
allocation, foreign dependency, C/C++ runtime, other-language standard
library, public ABI, or `topal-native/6` revision. Tests shall share the
interpreter/compiler source, verify both operand positions and once-only left
...
## TOPAL-COMP-LEXICAL-RETURN-CALL-001 — Named-call argument lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as a direct argument of
an unshadowed named function with exactly one flat declaration. It shall cover
the sole argument of a unary prefix call and either direct argument of a binary
infix call. A left or sole-argument exit shall precede every other argument. A
right exit shall evaluate the complete left application prefix exactly once,
abandon that value, and omit callee selection, invocation, later arguments, and
the enclosing function tail at O0.

The compiler shall validate the returned value against the enclosing function
result classifier, represent an evaluated prefix in the private exit sequence,
retain the block's nested DWARF scope, and use the existing private result and
single machine return. Overloaded, bound, nested, qualified, packaged,
defaulted, or constructor calls; products nested in arguments; decisions;
callbacks; generators; and cleanup-bearing scopes shall remain fail-closed.
The implementation shall add no runtime control-flow value, indirect call,
unwind edge, allocation, foreign dependency, C/C++ runtime, other-language
standard library, public ABI, or `topal-native/6` revision. Tests shall share
the interpreter/compiler source, cover unary, left, and right arguments, verify
once-only prefix evaluation and omitted invocation/tails, inspect
LLVM/DWARF/GDB behavior, and record separate resource baselines. This realizes
`TOPAL-FUNCTION-RETURN-001` and
`TOPAL-COMPILER-LEXICAL-RETURN-CALL-001` for increment 3b2-b5e8e.

## TOPAL-COMP-LEXICAL-RETURN-OPTIONAL-001 — Optional-constructor payload lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct payload of
the built-in unary `Some` constructor. It shall complete the enclosing function
before payload classification, omit Optional construction and the lexical and
function tails at O0, and validate the returned value against the enclosing
function result classifier rather than the Optional payload or abandoned
binding classifier.
...
machine return. Except for the strict unary constructors covered by
`TOPAL-COMP-LEXICAL-RETURN-CONSTRUCTOR-001`, other constructors, products or
...
implementation shall add no runtime control-flow value, Optional allocation,
indirect call, unwind edge, foreign dependency, C/C++ runtime, other-language
standard library, public ABI, or `topal-native/6` revision. Tests shall share
the interpreter/compiler source, verify omitted
payload classification, Optional construction, binding, and tails, inspect
LLVM/DWARF/GDB behavior, and record separate resource baselines. This realizes
`TOPAL-FUNCTION-RETURN-001` and
`TOPAL-COMPILER-LEXICAL-RETURN-OPTIONAL-001` for increment 3b2-b5e8f.

## TOPAL-COMP-LEXICAL-RETURN-CONSTRUCTOR-001 — Strict unary-constructor payload lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct operand of
the built-in unary `String`, `Int`, `Nat`, or `Rational` constructor or as the
direct payload of an already-declared payload-bearing Union alternative. It
shall complete the enclosing function before conversion, payload
classification, or constructor selection; omit constructed values, nominal
Union tags, abandoned bindings, and lexical and function tails at O0; and
...
`TOPAL-COMP-LEXICAL-RETURN-MODULAR-REDUCE-001`, and the direct unary List
...
`TOPAL-COMP-LEXICAL-RETURN-BOOLEAN-DECISION-ACTIONS-001`, and complete
Comparison-value decisions whose every action returns as covered by
`TOPAL-COMP-LEXICAL-RETURN-COMPARISON-VALUE-DECISION-ACTIONS-001`, and complete
...
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete List
decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, other
...
other-language standard library, public ABI, or `topal-native/6` revision.
Tests shall share the interpreter/compiler source, cover every admitted
constructor family, verify omitted conversion/construction/binding/tails,
inspect LLVM/DWARF/GDB behavior, and record separate resource baselines. This
realizes
`TOPAL-FUNCTION-RETURN-001` and
...
## TOPAL-COMP-LEXICAL-RETURN-CONSTRAINT-001 — Named-constraint operand lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct operand of
an already-declared named Int constraint. It shall validate constraint identity,
Int base, and binding declaration order first, then complete the enclosing
function before predicate evaluation or refined evidence construction; omit the
predicate branch, refined evidence, abandoned binding, and lexical and function
tails at O0; and validate the returned value against the enclosing function
result classifier.

The compiler shall retain the returned lexical block as the existing private
function result, preserve its nested DWARF scope, and use the existing single
machine return. Forward, unknown, non-Int, dynamically selected, qualified,
...
add no runtime control-flow value, constraint validation, allocation, indirect
call, unwind edge, foreign dependency, C/C++ runtime, other-language standard
library, public ABI, or `topal-native/6` revision. Tests shall share the
interpreter/compiler source, verify identity/base/order guards, omitted
predicate/evidence/binding/tails, rejection boundaries, LLVM/DWARF/GDB behavior,
and separate resource baselines. This realizes `TOPAL-FUNCTION-RETURN-001`,
`TOPAL-TYPE-CONSTRAINT-VALIDATE-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-CONSTRAINT-001` for increment 3b2-b5e8j.

## TOPAL-COMP-LEXICAL-RETURN-MODULAR-001 — Named-modular operand lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct operand of
an already-declared named `ModNat` or `ModInt` type. It shall validate modular
identity and declaration order first, then complete the enclosing function
before Int classification, range validation, dynamic Result creation, or
nominal-value construction; omit comparisons, Result and nominal construction,
the abandoned binding, and lexical and function tails at O0; and validate the
returned value against the enclosing function result classifier.
...
C/C++ runtime, other-language standard library, public ABI, or `topal-native/6`
revision. Tests shall share the interpreter/compiler source, verify
identity/order guards, omitted validation/construction/binding/tails, rejection
...
## TOPAL-COMP-LEXICAL-RETURN-COLLECT-001 — Unary List-collection source lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct source of
the built-in unary `collect source` operation. It shall select the exact
built-in operation first, then complete the enclosing function before
finite-traversal classification, generator consumption, or List materialization;
omit List nodes, consumption actions, the abandoned binding, and lexical and
function tails at O0; and validate the returned value against the enclosing
function result classifier.
...
runtime control-flow value, collection materialization, allocation, indirect
call, unwind edge, foreign dependency, C/C++ runtime, other-language standard
library, public ABI, or `topal-native/6` revision. Tests shall share the
interpreter/compiler source, verify exact operation selection, omitted
classification/materialization/binding/tails, rejection boundaries,
LLVM/DWARF/GDB behavior, and separate resource baselines. This realizes
`TOPAL-FUNCTION-RETURN-001`,
`TOPAL-COLLECTION-COLLECT-LIST-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-COLLECT-001` for increment 3b2-b5e8m.

## TOPAL-COMP-LEXICAL-RETURN-UNORDERED-COLLECT-001 — Unordered-collection source lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct source of
the exact built-in `collect-set source` or `collect-bag source` operation. It
shall select the exact built-in operation first, then complete the enclosing
function before finite-List classification, equality-dependent duplicate
coalescing, multiplicity accumulation, or Set or Bag materialization; omit
unordered collection nodes, comparisons, consumption actions, the abandoned
binding, and lexical and function tails at O0; and validate the returned value
against the enclosing function result classifier.
...
`TOPAL-BAG-COLLECT-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-UNORDERED-COLLECT-001` for increment 3b2-b5e8n.

## TOPAL-COMP-LEXICAL-RETURN-MAP-COLLECT-001 — Map-collection source lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct source of
the exact built-in `collect-map source resolving policy` operation when
`policy` is `reject`, `keep-first`, or `keep-last`. It shall select the exact
operation, `resolving` clause, and valid collision policy first, then complete
the enclosing function before pair classification, key equality, collision
resolution, or Map materialization; omit Map nodes, comparisons, collision
actions, the abandoned binding, and lexical and function tails at O0; and
validate the returned value against the enclosing function result classifier.
...
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete List
decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, callbacks,
...
## TOPAL-COMP-LEXICAL-RETURN-EXHAUSTIVE-BOOLEAN-SUBJECT-001 — Exhaustive Boolean-decision subject lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct subject of
a complete Boolean decision consisting of exactly one `false` rule and one
`true` rule, in either source order and without `otherwise`. It shall recognize
that exact two-rule matcher set first, then complete the enclosing function
before Boolean classification, matcher consideration, action selection, or
action evaluation; omit decision branches, actions, the abandoned decision
value, and lexical and function tails at O0; and validate the returned value
against the enclosing function result classifier.
...
allocation, indirect call, unwind edge, foreign dependency, C/C++ runtime,
other-language standard library, public ABI, or `topal-native/6` revision.
Tests shall share the interpreter/compiler source, verify exact matcher-set
selection in both source orders, omitted classification/matching/action/tail
work, rejection boundaries, LLVM/DWARF/GDB behavior, and separate resource
baselines. This realizes `TOPAL-FUNCTION-RETURN-001`,
`TOPAL-DECISION-BOOLEAN-001`, and
...
## TOPAL-COMP-LEXICAL-RETURN-COMPLETE-DECISION-SUBJECT-001 — Parser-complete decision subject lexical exit

Inside an admitted ordinary function, the checked model shall propagate an
explicit return from a cleanup-free lexical block used as the direct subject of
any decision table already accepted as complete by the syntax rules. It shall
use construction of the complete decision-table syntax node as prior evidence,
then complete the enclosing function before subject classification,
type-specific exhaustiveness or matcher validation, matcher-operand evaluation,
pattern binding, comparison, action selection, or action evaluation; omit all
matcher, binding, branch, action, abandoned-value, and lexical and function
tail work at O0; and validate the returned value against the enclosing function
result classifier.
...
indirect call, unwind edge, foreign dependency, C/C++ runtime, other-language
standard library, public ABI, or `topal-native/6` revision. Tests shall share
the interpreter/compiler source, exercise complete no-fallback Optional,
Result, List, and identifier matcher shapes, verify omitted
subject/matcher/binding/action/tail work, rejection boundaries,
LLVM/DWARF/GDB behavior, and separate resource baselines. This realizes
`TOPAL-FUNCTION-RETURN-001`, `TOPAL-DECISION-OPTIONAL-001`,
`TOPAL-DECISION-RESULT-001`, `TOPAL-DECISION-LIST-001`, and
...
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete List
decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, nested action
...
allocation, indirect call, unwind edge, foreign dependency, C/C++ runtime,
other-language standard library, public ABI, or `topal-native/6` revision.
Tests shall share the interpreter/compiler source,
...
## TOPAL-COMP-LEXICAL-RETURN-COMPARISON-VALUE-DECISION-ACTIONS-001 — Complete Comparison action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from the selected action of a complete
decision over the closed `Comparison` value when every action is a direct
cleanup-free returning lexical block. They shall evaluate the subject once,
consider `Less`, `Equal`, `Greater`, and final-fallback matchers in source order,
evaluate only the selected action, validate its returned value against the
enclosing function result classifier, and omit the selected block tail and
following function tail at O0.

The compiler shall retain the Comparison-value decision as the function result,
...
foreign dependency, C/C++ runtime, other-language standard library, public ABI,
or `topal-native/6` revision. Tests shall share the interpreter/compiler source,
exercise all three Comparison alternatives and a final-fallback form, verify
exact selection and skipped tails, retain mixed-path rejection, inspect the LLVM
switch/phi/single-return shape and DWARF/GDB behavior, and record separate
resource baselines. This realizes `TOPAL-FUNCTION-RETURN-001`,
`TOPAL-DECISION-ENUM-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-COMPARISON-VALUE-DECISION-ACTIONS-001` for
increment 3b2-b5e8w.

## TOPAL-COMP-LEXICAL-RETURN-ORDERED-COMPARISON-DECISION-ACTIONS-001 — Ordered comparison action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from the selected action of an ordered
comparison decision with a nonempty comparison-rule prefix and final
`otherwise` when every action is a direct cleanup-free returning lexical block.
They shall evaluate the exact numeric subject once, evaluate matcher operands
and consider rules in source order until selection, evaluate only the selected
...
The compiler shall retain the ordered comparison decision as the function
result, retain each returning lexical block as its branch value, preserve each
nested DWARF scope, emit the ordinary ordered comparison and branch chain with a
...
allocation, indirect call, unwind edge, foreign dependency, C/C++ runtime,
other-language standard library, public ABI, or `topal-native/6` revision.
Tests shall share the interpreter/compiler source, exercise multiple comparison
rules and the final fallback, verify source-order matching and skipped tails,
retain mixed-path rejection, inspect the LLVM comparison-chain/phi/single-return
shape and DWARF/GDB behavior, and record separate resource baselines. This
realizes `TOPAL-FUNCTION-RETURN-001`, `TOPAL-DECISION-COMPARISON-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-ORDERED-COMPARISON-DECISION-ACTIONS-001` for
increment 3b2-b5e8x.

## TOPAL-COMP-LEXICAL-RETURN-ENUM-FALLBACK-DECISION-ACTIONS-001 — Final-fallback Enum action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from the selected action of a decision
over an already-declared non-Comparison Enum when the decision has a nonempty
prefix of distinct identifier matchers followed by final `otherwise` and every
action is a direct cleanup-free returning lexical block. They shall evaluate
the Enum subject once, consider alternatives in source order, evaluate only the
selected action, validate its returned value against the enclosing function
result classifier, and omit the selected block tail and following function tail
at O0.

The compiler shall retain the Enum decision as the function result, retain each
...
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, and complete List
decisions whose every action returns as admitted by
`TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, other decision
...
indirect call, unwind edge, foreign dependency, C/C++ runtime, other-language
standard library, public ABI, or `topal-native/6` revision. Tests shall share
the interpreter/compiler source, exercise explicit alternatives and fallback,
verify source-order selection and skipped tails, retain mixed-path and unknown
alternative rejection, inspect the LLVM switch/phi/single-return shape and
DWARF/GDB behavior, and record separate resource baselines. This realizes
...
## TOPAL-COMP-LEXICAL-RETURN-OPTIONAL-DECISION-ACTIONS-001 — Complete Optional action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from the selected action of a complete
decision over `Optional T` when every action is a direct cleanup-free returning
lexical block. The admitted matcher shapes are distinct `Some name` and `None`
rules in either order, either explicit alternative followed by final
`otherwise`, and `otherwise` alone. Both execution paths shall evaluate the
subject once, consider rules in source order, bind the present payload only in
the selected explicit `Some name` action, evaluate only that action, validate
its returned value against the enclosing function result classifier, and omit
the selected block tail and following function tail at O0.

The compiler shall retain the Optional decision as the function result, retain
each returning lexical block as its branch value, preserve each nested DWARF
scope and the selected payload binding, emit the ordinary Optional tag query,
two-way branch, conditional payload extraction, and typed SSA join, and use the
existing single machine return. Duplicate or incomplete alternatives, mixed or
additional matcher kinds, nonfinal fallbacks, non-Optional subjects, any
normally completing action, a complete Result decision outside
`TOPAL-COMP-LEXICAL-RETURN-RESULT-DECISION-ACTIONS-001` or
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, a complete List
decision outside `TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001`, or other decision
classifiers, nested action expressions, callbacks, generators, and
cleanup-bearing actions shall remain fail-closed. The implementation shall
add no runtime control-flow value, allocation, indirect call, unwind edge,
foreign dependency, C/C++ runtime, other-language standard library, public ABI,
or `topal-native/6` revision. Tests shall share the interpreter/compiler source,
exercise present payload binding, absent matching, both one-alternative fallback
shapes, source-order selection, skipped tails, mixed-path and non-Optional
rejection, exact LLVM branch/phi/single-return behavior, nested DWARF, GDB values
and frames, the complete compiler corpus, and separate resource baselines. This
realizes `TOPAL-FUNCTION-RETURN-001`, `TOPAL-DECISION-OPTIONAL-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-OPTIONAL-DECISION-ACTIONS-001` for increment
3b2-b5e8aa.

## TOPAL-COMP-LEXICAL-RETURN-RESULT-DECISION-ACTIONS-001 — Exhaustive Result action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from the selected action of an
exhaustive `Result (T, Codes)` decision when its exact `Ok value` and `Error
problem` actions are direct cleanup-free returning lexical blocks. Both source
orders shall be admitted. The interpreter and compiler shall evaluate the
subject once, consider rules in source order, bind the selected success value or
structured Error only inside its action, evaluate only that action, validate the
returned value against the enclosing function result classifier, and omit the
selected block tail and following function tail at O0.

The compiler shall retain the Result decision as the function result, retain
each returning lexical block as its branch value, preserve each nested DWARF
scope and both path-local payload bindings, emit the ordinary Result tag query,
two-way branch, payload extraction, and typed SSA join, and use the existing
single machine return. Qualified Error-code matcher tables outside
`TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001`, duplicate,
missing, mixed, or additional matchers, non-Result subjects, any normally
...
dependency, C/C++ runtime, other-language standard library, public ABI, or
`topal-native/6` revision. Tests shall share the interpreter/compiler source,
exercise success and structured-Error bindings plus both matcher orders,
verify once-only source-order selection and skipped tails, retain mixed-path and non-Result
rejection, inspect exact LLVM branch/phi/single-return behavior and nested
DWARF/GDB values and frames, run the complete compiler corpus, and record
separate resource baselines. This realizes `TOPAL-FUNCTION-RETURN-001`,
`TOPAL-DECISION-RESULT-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-RESULT-DECISION-ACTIONS-001` for increment
3b2-b5e8ab.

## TOPAL-COMP-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001 — Qualified Error-code action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from every action of a complete
decision over `Result (T, lang arithmetic ArithmeticErrorCode)`. The table
shall contain exactly one `Ok value` rule and at least one distinct qualified
arithmetic Error-code rule, followed either by a whole-`Error problem` fallback
or, without a fallback, by exact coverage of all four closed arithmetic codes.
Every action shall be a direct cleanup-free returning lexical block.

The interpreter and compiler shall evaluate the subject once, preserve
source-order rule consideration, bind the success value or whole Error only in
a selected binding matcher, evaluate only the selected action, validate its
returned value against the enclosing function result classifier, and omit the
selected block tail and following function tail at O0. Qualified code actions
shall not bind the whole Error. The checked model shall retain ordered nominal
codes, the optional fallback, and normalized branch values. LLVM shall emit the
ordinary Result tag query, path-local payload extraction, one code query and
switch, a fail-closed unreachable default for exact no-fallback coverage, a
typed SSA join, nested action DWARF and selected payload bindings, and the
existing single machine return.

Unknown, duplicate, missing, post-fallback, mixed, or additional matchers;
non-arithmetic or multi-vocabulary code sets; non-Result subjects; normally
completing actions; nested action expressions; callbacks; generators; and
cleanup-bearing forms shall remain fail-closed. The implementation shall add no
runtime control-flow value, allocation, indirect call, unwind edge, foreign
dependency, C/C++ runtime, other-language standard library, public ABI, or
`topal-native/6` revision. Tests shall share one interpreter/compiler source,
exercise success, selected code, whole-Error fallback, and exact four-code
coverage, verify skipped tails plus mixed-path and non-Result rejection, inspect
the exact LLVM switches/joins/single returns and fail-closed default, preserve
DWARF/GDB success and Error values and frames, run the complete compiler corpus,
and record separate resource baselines. This realizes
`TOPAL-FUNCTION-RETURN-001`, `TOPAL-DECISION-RESULT-001`,
`TOPAL-DECISION-ERROR-CODE-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-ERROR-CODE-DECISION-ACTIONS-001` for increment
3b2-b5e8ac.

## TOPAL-COMP-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001 — Complete List action lexical exits

Inside an admitted ordinary function, the interpreter and checked compiler
model shall propagate an explicit return from both actions of a complete List
decision. The table shall contain exactly one `Entry (first, rest)` rule and
one `Empty` rule in either order, the entry names shall be distinct, and both
actions shall be direct cleanup-free returning lexical blocks.

The interpreter and compiler shall evaluate the subject once, preserve
source-order rule consideration, bind the first entry and remaining List only
for the selected Entry action, evaluate only the selected action, validate its
returned value against the enclosing function result classifier, and omit the
selected block tail and following function tail at O0. The checked model shall
retain the List decision, entry bindings, supported exact element facts, and
normalized branch values. LLVM shall emit the ordinary empty test,
path-local head/rest decomposition, two action edges, a typed SSA join, nested
action and binding DWARF, and the existing single machine return.

Duplicate binding names, missing, duplicate, mixed, or additional matchers;
non-List subjects; unsupported List element representations; normally
completing actions; nested action expressions; callbacks; generators; and
cleanup-bearing forms shall remain fail-closed. The implementation shall add no
runtime control-flow value, allocation beyond the subject's existing List
representation, indirect call, unwind edge, foreign dependency, C/C++ runtime,
other-language standard library, public ABI, or `topal-native/6` revision.
Tests shall share one interpreter/compiler source, exercise Entry and Empty
selection plus both matcher orders, verify skipped tails and mixed-path,
non-List, and duplicate-binding rejection, inspect exact LLVM branching,
decomposition, join, and single return, preserve DWARF/GDB bindings and frames,
run the complete compiler corpus, and record separate resource baselines. This
realizes `TOPAL-FUNCTION-RETURN-001`, `TOPAL-DECISION-LIST-001`, and
`TOPAL-COMPILER-LEXICAL-RETURN-LIST-DECISION-ACTIONS-001` for increment
3b2-b5e8ad.
...
## TOPAL-COMP-LIST-OPTIONAL-INT-CORE-001 — Ordinary Optional Int Lists
...
emptiness, display, and debugging for `List Optional Int`. Linux x86-64 shall
use null List Empty and immutable 16-byte Optional-pointer/remaining-pointer
nodes; a conditional finite fragment shall delegate exact element equality to
the existing Optional-Int runtime and count correctly at O0. LLVM owns physical
placement, while DWARF/GDB retain both classifiers.
...
baselines. This adds no flattened Optional, copied Int, generic/public/foreign
ABI, foreign allocation, C/C++, other-language library, or ABI revision. Future
...
## TOPAL-COMP-LIST-OPTIONAL-STRING-CORE-001 — Ordinary Optional String Lists

The checked model shall admit construction, private parameter/result/package
and Tuple/Record passage, derived equality, complete decisions, count,
emptiness, display, and debugging for `List Optional String`. Linux x86-64 shall
use null List Empty and immutable 16-byte Optional-pointer/remaining-pointer
nodes; a conditional finite fragment shall delegate exact element equality to
the existing Optional-String runtime and count correctly at O0. LLVM owns
physical placement, while DWARF/GDB retain all classifier layers.

Tests cover `None`, empty/ASCII/non-ASCII `Some`, same/different content and
length, all boundaries, rejection, IR, ELF/DWARF, GDB, corpus, history, and
separate baselines. This adds no flattened Optional, copied String descriptor,
generic/public/foreign ABI, foreign allocation, C/C++, other-language library,
or ABI revision. Future metadata remains independent of private headers and
nodes. This realizes `TOPAL-COMPILER-LIST-OPTIONAL-STRING-CORE-001` for increment
4b3d-ad.
...
## TOPAL-COMP-LIST-STRING-PAIR-CORE-001 — Ordinary String-pair Lists

The checked model shall admit construction, private parameter/result/package
and nested Tuple/Record passage, derived equality, complete decisions, count,
emptiness, display, and debugging for `List (String, String)`. Linux x86-64
shall use immutable 24-byte String-pointer/String-pointer/remaining-pointer
nodes. A conditional finite fragment shall compare both exact fields in source
order and count correctly at O0. LLVM owns physical placement, while DWARF/GDB
retain the recursive List and positional Tuple classifiers.

Tests cover empty, non-ASCII, and non-Latin Strings, same/different fields and
length, all private boundaries, rejection, IR, ELF/DWARF, GDB, corpus, history,
and separate baselines. This adds no Tuple allocation, generic/public/foreign
ABI, foreign allocation, C/C++, other-language library, or ABI revision. Future
metadata remains independent of private nodes and offsets. This realizes
`TOPAL-COMPILER-LIST-STRING-PAIR-CORE-001` for increment 4b3d-ah.
