...
fn every_mode_applies_compound_packaged_operands() {
    ...
    // TOPAL-TYPE-CALL-001
    let source = include_str!("../../../examples/language/compound-packaged-function-operands.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_applies_structured_packaged_fields() {
    ...
    // TOPAL-TYPE-CALL-001
    let source = include_str!("../../../examples/language/structured-packaged-function-fields.t");
...
fn every_mode_returns_from_direct_optional_constructor_arguments() {
    ...
    let source = include_str!("../../../examples/language/function-return-optional-constructor.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_returns_from_direct_positional_variant_arguments() {
    ...
    let source = include_str!("../../../examples/language/function-return-variant-constructor.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_returns_from_direct_character_constructor_arguments() {
    ...
    let source = include_str!("../../../examples/language/function-return-character-constructor.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_returns_from_direct_named_modular_arguments() {
    ...
    let source = include_str!("../../../examples/language/function-return-modular-constructor.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_returns_from_final_fallback_enum_decision_actions() {
    ...
    let source =
        include_str!("../../../examples/language/function-return-enum-fallback-decision-actions.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_returns_from_list_decision_actions() {
    ...
    let source = include_str!("../../../examples/language/function-return-list-decision-actions.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(
            output.status.success(),
...
fn every_mode_requires_exact_repeated_function_aggregate_identity() {
    ...
    // TOPAL-TYPE-MATCH-001, TOPAL-FUNCTION-ANONYMOUS-001
    let source = include_str!("../../../examples/language/repeated-function-aggregate-patterns.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(output.status.success());
        let stdout = String::from_utf8(output.stdout).unwrap();
...
fn every_mode_requires_exact_repeated_captured_function_identity() {
    ...
    // TOPAL-TYPE-MATCH-001, TOPAL-FUNCTION-ANONYMOUS-001
    let source = include_str!("../../../examples/language/repeated-captured-function-patterns.t");
    for arguments in [&[][..], &["--interactive"][..], &["--test"][..]] {
        let output = run(arguments, source);
        assert!(output.status.success());
        let stdout = String::from_utf8(output.stdout).unwrap();
