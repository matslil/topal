...
body statement, discard, binding initializer, the whole operand of another
explicit return, or a direct symbolic-operator operand, an explicit return
shall propagate to that function boundary under `TOPAL-FUNCTION-RETURN-001`
without evaluating either the remaining block statements or the enclosing
function tail. A left operator exit shall omit the right operand; a right
operator exit shall evaluate the left application prefix once and omit the
operator. An outer return shall not create a second explicit-return decision.
Test traces and debugger history shall retain block evaluation and the inner
...
## TOPAL-INTP-SUBSET-296 — Ordinary Optional String List values

All source modes shall preserve `None` and exact UTF-8 String-bearing `Some`
...
## TOPAL-INTP-SUBSET-299 — Ordinary String/Int-pair List values

All source modes shall preserve exact immutable String and arbitrary-precision
Int fields, including empty and non-ASCII Strings and signed large Ints, through
